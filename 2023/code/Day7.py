...
for index, card in enumerate(all_cards_split):
    ...
    for single in values:
        card_dict[single] += 1
    j_count = int(card_dict["J"])
    del card_dict["J"]
    listed_dict = list(map(list, card_dict.items()))
    listed_dict.sort(key=lambda x: x[1])
    if j_count != 5:
        listed_dict[-1][1] += j_count
    else:
        listed_dict = [["J", 5]]
    after_J_values = [x[1] for x in listed_dict]
    match after_J_values:
        case [5]:
            curr_number = "7"
        case [1,4]:
            curr_number = "6"
